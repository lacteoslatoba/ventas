...
function TicketPreview({ config }) {
        ...
        return (
            <div className="bg-white border-2 border-dashed border-slate-200 rounded-2xl p-4 flex justify-center">
                ...
                >
                    ...
                    {showCustomer && <div style={metaStyle} className={metadataAlignment === 'between' ? 'flex justify-between' : `text-${metadataAlignment}`}><span>{showLabels ? 'Cliente' : ''}</span><span>Tienda La Fe</span></div>}
                    {showSeller && <div style={metaStyle} className={metadataAlignment === 'between' ? 'flex justify-between' : `text-${metadataAlignment}`}><span>{showLabels ? 'Repartidor' : ''}</span><span>Juan Pérez</span></div>}
                    ...
                    <div className="uppercase text-[9px] mt-1 opacity-70">ARTICULOS: 2</div>
                    
                    {showSubtotal && <div className="text-right mt-1 font-bold uppercase text-[9px]">SUBTOTAL: $165.00</div>}
                    
                    {showSeparatorFooter && <Separator />}

                    <div className={`${centerTotal ? 'text-center' : 'text-right'} leading-[1.2] mt-1 mb-1 uppercase`} style={{ fontSize: `${totalFontSize}px`, fontWeight: totalBold ? 'bold' : 'normal' }}>TOTAL $165.00</div>
    ...
    return (
        <div className="bg-white border-2 border-dashed border-slate-200 rounded-2xl p-4 flex justify-center">
            <div
                className={`font-mono leading-snug text-black bg-white overflow-hidden`}
                style={{ width: '200px', minHeight: '300px', fontSize: `${useFontB ? 8.5 : 9.5}px` }}
            >
                {showLogo && logoUrl && (
                    <div className="flex justify-center mb-2">
                        <img src={logoUrl} alt="Logo" className="max-w-[100px] max-h-[60px] object-contain grayscale" />
                ...
                <div style={metaStyle}>
                    ...
                    {showSeller && (
                        <div className={metadataAlignment === 'between' ? 'flex justify-between' : `text-${metadataAlignment}`}>
                            <span>{showLabels ? 'Vendedor:' : ''}</span> <span>Juan Pérez</span>
                        </div>
                    )}
                    {showCustomer && (
                        <div className={metadataAlignment === 'between' ? 'flex justify-between' : `text-${metadataAlignment}`}>
                ...
                {showSubtotal && <div className="text-right text-[9px] opacity-70 uppercase">SUBTOTAL $165.00</div>}
                <div className={`${centerTotal ? 'text-center' : 'text-right'} leading-[1.2] uppercase`} style={{ fontSize: `${totalFontSize}px`, fontWeight: totalBold ? 'bold' : 'normal' }}>TOTAL $165.00</div>
