            ...
            {isFormOpen && (
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 mb-8">
                    ...
                    <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                        <div className={currentUser?.role === 'admin' ? "md:col-span-3" : ""}>
                ...
                <table className="w-full text-left">
                    <thead>
                        <tr className="border-b-2 border-slate-200">
                            ...
                            {currentUser?.role !== 'admin' && <th className="py-4 px-2 font-black text-slate-400 uppercase tracking-widest text-[10px]">Teléfono</th>}
                            {currentUser?.role !== 'admin' && <th className="py-4 px-2 font-black text-slate-400 uppercase tracking-widest text-[10px]">Lugar</th>}
                            <th className="py-4 px-2 font-black text-slate-400 uppercase tracking-widest text-[10px] text-right">Acciones</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                        {visibleClients.length === 0 && <tr><td colSpan={currentUser?.role === 'admin' ? "2" : "4"} className="text-center py-16 text-slate-400 italic font-medium">Sin clientes registrados</td></tr>}
                        {visibleClients.map((c) => (
                            <tr key={c.id} className="hover:bg-primary/5 transition-colors group">
                                ...
                                {currentUser?.role !== 'admin' && <td className="py-4 px-2 text-slate-500 font-medium">{c.phone || '-'}</td>}
                                {currentUser?.role !== 'admin' && <td className="py-4 px-2 text-slate-500 font-medium">{c.address || '-'}</td>}
            ...
            <div className="md:hidden space-y-4">
                ...
                {visibleClients.map((c) => (
                    <div key={c.id} className="py-5 border-b border-slate-100 relative group">
                        <div className="flex justify-between items-start">
                            <div className="flex-1 pr-4">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {currentUser?.role !== 'admin' && c.phone && <span className="text-slate-500 text-[10px] font-bold flex items-center gap-1 bg-white/50 px-2 py-0.5 rounded-lg border border-slate-100">{c.phone}</span>}
                                </div>
                            </div>
                        ...
                        {currentUser?.role !== 'admin' && c.address && (
